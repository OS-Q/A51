...
function install_platformio()
{
    ...
    python -c "import json; import os; fp=open(os.path.expanduser('~/.platformio/platforms/espressif8266/platform.json'), 'r+'); data=json.load(fp); data['packages']['A15']['version'] = '*'; fp.seek(0); fp.truncate(); json.dump(data, fp); fp.close()"
    ln -sf $TRAVIS_BUILD_DIR ~/.platformio/packages/A15
